...
class AbstractService(ABC):
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        ...
        # WebUI集成
        self.webui_enabled = getattr(config, 'webui', {}).get('enabled', False)
        self.webui_service = None  # 由launcher设置
    ...
    async def send_webui_data(self, data_type: str, symbol: str, data: Dict[str, Any]):
        """通用WebUI数据推送方法"""
        if not self.webui_enabled or not self.webui_service:
            return
        
        webui_message = {
            'service': self.name,
            'data_type': data_type,  # 'kline', 'signal', 'progress', 'metrics'
            'symbol': symbol,
        ...
        try:
            await self.webui_service.receive_data(webui_message)
        except Exception as e:
            self.log("DEBUG", f"WebUI数据推送失败: {e}")
...
class AbstractStrategy(AbstractService):
    ...
    def __init__(self, strategy_id: str, config: Dict[str, Any]):
        ...
        self.execution_interval = config.get("execution_interval_seconds", 900)  # 默认15分钟

    async def async_run(self):
        """
        Main strategy loop.
        
        Simple timer-based execution - no global coordination needed.
        """
