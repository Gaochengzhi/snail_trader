...
class SchedulerService(AbstractService):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.step_interval = config.get('global_step_interval_minutes', 5) * 60  # Convert to seconds
        self.current_step = 0
    ...
    async def initialize(self):
        ...
        if self.mode == 'backtest':
            ...
            self.message_bus.register_handler(Topics.DATA_PROCESSED, self._handle_data_processed)
    ...
    async def _run_live_mode(self):
        ...
        """
        self.logger.info(f"Scheduler running in live mode, interval: {self.step_interval}s")
