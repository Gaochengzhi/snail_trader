...
class Config:
    """抽象的配置管理类 - 支持变量引用、环境覆盖和动态配置源"""

    def __init__(self, config_name: Optional[str] = None, config_sources: Optional[List[str]] = None):
        self._project_root = Path(__file__).resolve().parent.parent
        self._cfg_dir = self._project_root / "configs"
        self._settings: Optional[StrategySettings] = None
        self._config_name = config_name or self._get_config_from_args()
        self._config_sources = config_sources or []
    ...
    def load_additional_config(self, config_path: str) -> DotDict:
        """加载额外的配置文件并与当前配置合并"""
        # 解析配置文件路径
        if Path(config_path).is_absolute():
            additional_path = Path(config_path)
        else:
            additional_path = self._project_root / config_path
        
        if not additional_path.exists():
            raise FileNotFoundError(f"Configuration file not found: {additional_path}")
        
        # 获取当前基础配置
        base_config = self._load_yaml(self._cfg_dir / "base.yaml")
        if self._config_name:
            env_config = self._load_yaml(self._cfg_dir / f"{self._config_name}.yaml")
            base_config = self._merge_configs(base_config, env_config)
        
        # 加载并合并额外配置
        additional_config = self._load_yaml(additional_path)
        merged_config = self._merge_configs(base_config, additional_config)
    ...
    def merge_configs(self, *config_paths: str) -> DotDict:
        """合并多个配置文件"""
        # 从基础配置开始
        merged_config = self._load_yaml(self._cfg_dir / "base.yaml")
        ...
        for config_path in config_paths:
            if Path(config_path).is_absolute():
                ...
                path = self._project_root / config_path
